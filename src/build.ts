...
const TENSORRT_RTX_ARCHIVE_URL =
  platform === "linux"
...
await new Command()
  ...
  .option(
    "-v, --upstream-version <version:string>",
    "Exact version of upstream package",
    { required: true },
  )
  ...
  .option(
    "-A, --arch <arch:target-arch>",
    "Configure target architecture for cross-compile",
    { default: "x86_64" },
  )
  .action(async (options, ..._) => {
    ...
    if (isExists) {
      $.cd(onnxruntimeRoot);
      const currentBranch = (
        await $`git branch --show-current`.stdout("piped")
      ).stdout.trim();
      ...
      if (!isBranchCorrect) {
        console.log(
          `Removing onnxruntime directory because branch is incorrect: ${onnxruntimeRoot}, current branch: ${currentBranch}, expected branch: rel-${options.upstreamVersion}`,
        );
    ...
    if (platform === "linux" && !options.android) {
      if (options.arch === "aarch64") {
        ...
        if (options.cuda) {
          ...
          if (!hostCompiler) {
            console.error(
              "Warning: Could not find aarch64-linux-gnu-g++. CUDA build may fail.",
            );
          ...
          try {
            ...
            if (nvccPath) {
              ...
              if (!(await exists(join(targetProfileDir, "include")))) {
                console.log(
                  "Patching CUDA: Creating target include directory symlink...",
                );
              ...
              if (!(await exists(libDir))) {
                ...
                const libsToDownload = [
                  {
                    ...
                    extractSubDir:
                      "cuda_cudart-linux-aarch64-12.8.57-archive/lib",
                  },
                  {
                    name: "libcublas",
                    ...
                    extractSubDir:
                      "libcublas-linux-aarch64-12.8.3.14-archive/lib",
                  },
                  {
                    name: "libcufft",
                    ...
                    extractSubDir:
                      "libcufft-linux-aarch64-11.3.3.41-archive/lib",
                  },
                  {
                    name: "libcurand",
                ...
                if (!(await exists(stubsDir))) {
                  ...
                  // 复制 stub 库（用于链接时）
                  for (const lib of libsToDownload) {
                    const extractedStubsDir = join(
                      tmpDir,
                      lib.extractSubDir.replace("/lib", "/lib/stubs"),
                    );
                    try {
              ...
              args.push(
                `-DCMAKE_CUDA_COMPILER_LIBRARY_ROOT=${targetProfileDir}`,
              );
    ...
    if (
      platform === "darwin" &&
      (options.iphoneos || options.iphonesimulator)
    ) {
      args.push(
        `-DCMAKE_OSX_DEPLOYMENT_TARGET=${Deno.env.get("IPHONEOS_DEPLOYMENT_TARGET")}`,
      );
      args.push(
    ...
    if (platform === "linux" && options.android) {
      ...
      args.push(
        `-DCMAKE_TOOLCHAIN_FILE=${join(Deno.env.get("ANDROID_NDK_HOME")!, "build", "cmake", "android.toolchain.cmake")}`,
      );
    }
    ...
    if (options.cuda) {
      ...
      if (!should_skip) {
        ...
        const cudnnArchiveStream = await fetch(cudnnArchiveUrl).then(
          (c) => c.body!,
        );
        await Deno.mkdir(cudnnOutPath);
        await $`tar xvJC ${cudnnOutPath} --strip-components=1 -f -`.stdin(
          cudnnArchiveStream,
        );
      }
    ...
    if (options.trt) {
      const trtArchiveStream = await fetch(TENSORRT_ARCHIVE_URL).then(
        (c) => c.body!,
      );
      const trtOutPath = join(root, "tensorrt");
      await Deno.mkdir(trtOutPath);
      await $`tar xvzC ${trtOutPath} --strip-components=1 -f -`.stdin(
        trtArchiveStream,
      );
      args.push(`-Donnxruntime_TENSORRT_HOME=${trtOutPath}`);
    }
    if (options.nvrtx) {
      const trtxArchiveStream = await fetch(TENSORRT_RTX_ARCHIVE_URL).then(
        (c) => c.body!,
      );
      const trtxOutPath = join(root, "tensorrt");
      await Deno.mkdir(trtxOutPath);
      await $`tar xvzC ${trtxOutPath} --strip-components=1 -f -`.stdin(
        trtxArchiveStream,
      );
      ...
      if (options.arch === "aarch64" && arch !== "arm64") {
        ...
        switch (platform) {
          ...
          case "linux":
            if (!options.android) {
              args.push(
                `-DCMAKE_TOOLCHAIN_FILE=${join(root, "toolchains", crossToolchainFile)}`,
              );
            }
            break;
        }
    ...
    args.push(
      `-Donnxruntime_USE_KLEIDIAI=${options.arch === "aarch64" ? "ON" : "OFF"}`,
    );
    ...
    if (
      options.ninja &&
      !(platform === "win32" && options.arch === "aarch64")
    ) {
      args.push("-G", "Ninja");
    }
    ...
    const sourceDir = options.static
      ? join(root, "src", "static-build")
      : "cmake";
    const artifactOutDir = join(root, "artifact", "onnxruntime");

    await $`cmake -S ${sourceDir} -B build -D CMAKE_BUILD_TYPE=Release -DCMAKE_CONFIGURATION_TYPES=Release -DCMAKE_INSTALL_PREFIX=${artifactOutDir} -DONNXRUNTIME_SOURCE_DIR=${onnxruntimeRoot} --compile-no-warning-as-error ${args}`.env(
      env,
    );
